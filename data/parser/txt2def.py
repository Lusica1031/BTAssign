...
def writeDef(out_def:str, design_name:str, d_x:int, d_y:int, top_comp_cnt:int, bot_comp_cnt:int, terminal_cnt:int):
    ...
    if(ENABLE_MERGE):
        ...
        f_out_bot.write("\nTRACKS Y 3340 DO 146 STEP 3200 LAYER metal10 ;\n")
    ...
    for inst_name, inst_info in inst_infos.items():
        ...
        if(inst_info["tier"]):
            # top
            f_out_top.write("- {} {} + PLACED ( {:d} {:d} ) N\n ;\n".format(inst_name, inst_info["type"], inst_info["loc_x"], inst_info["loc_y"]))
        else:
            # bottom
            if(ENABLE_MERGE):
                f_out_top.write("- {} {} + PLACED ( {:d} {:d} ) N\n ;\n".format(inst_name, inst_info["type"]+"_45", inst_info["loc_x"], inst_info["loc_y"]))
            else:
        ...
        while line:
            ...
            if(cell_flag):
                ...
                if(pin_flag and " RECT " in line):
                    ...
                    pin_y = (y_l + y_r) / 2.0
                    if(tech_name == "TA"):
                        cell_infos[tech_name][cell_name]["pins"]["P"+str(P_cnt)] = {"loc_x": int(pin_x * 2000), "loc_y": int(pin_y * 2000), "name": pin_name}
                    pin_flag = False
                    P_cnt += 1
        ...
        while line:
            ...
            if("TopDiePlacement" in line):
                ...
                if("Inst" in line):
                    ...
                    inst_infos[lines[1]]["loc_x"] = int(lines[2])
                    inst_infos[lines[1]]["loc_y"] = int(lines[3])
            elif(bottom_flag):
                if("Inst" in line):
                    lines = line.split()
                ...
                if("Terminal" in line):
                    lines = line.split()
                    net_infos[lines[1]]["terminal"] = {"name": "T"+str(terminal_cnt), "loc_x": int(lines[2]), "loc_y": int(lines[3])}
                    terminal_cnt += 1
